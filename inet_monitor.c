...
//  inet_monitor
//
//  Created by Ke Wang on 10/23/17.
//  Copyright © 2017 Ke Wang. All rights reserved.
//
...
int parse_diag_msg(struct inet_diag_msg *diag_msg, int rtalen, int idle, int intv_in_ms, int* activeflow, int* newflow){
    struct rtattr *attr;
...
int query(int idle_thr, int port, int intv_in_ms, int* active_flow, int* new_flow){
    ...
    *new_flow = 0;
    ...
    while(1){
        ...
        while(NLMSG_OK(nlh, numbytes)){
            ...
            int tmp = parse_diag_msg(diag_msg, rtalen, idle_thr, intv_in_ms, active_flow, new_flow);
            if(tmp == -1)
                return -1;
